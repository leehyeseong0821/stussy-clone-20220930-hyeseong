...
class ProductDetail {
  ...
  loadProductDetail(responseData) {
    ...
    document.querySelector(".simple-info").innerHTML =
      responseData.pdtSimpleInfo;
    document.querySelector(
      ".detail-info"
    ).innerHTML = `<strong>PRODUCT DETAILS</strong>
${responseData.pdtDetailInfo}`;
    document.querySelector(".option-info").innerHTML =
      responseData.pdtOptionInfo;
    document.querySelector(".management-info").innerHTML =
      responseData.pdtManagementInfo;
    document.querySelector(".shipping-info").innerHTML =
      responseData.pdtShippingInfo;
  }
        ...
        entry[1].forEach((value) => {
          productSizes.innerHTML += `
            ...
            <input type="radio" id="product-size-${
              value.sizeName
            }"class="product-size" name="pdtSize" value="${value.sizeId}"${
            value.pdtStock == 0 ? disabled : ""
          }>
            <label for="product-size-${value.sizeName}" ${
            value.pdtStock == 0 ? 'class:"no-stock"' : ""
          }>${value.sizeName}</label> `;
        });
      }
    });
